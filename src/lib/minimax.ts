...
export interface MiniMaxChatRequest {
  model: 'MiniMax-M2.5' | 'MiniMax-M2.7' | 'MiniMax-M3'
  max_tokens?: number
  temperature?: number
...
export async function requestMiniMaxChat(
  ...
  const response = await fetch(`${EDGE_FUNCTIONS_URL}/minimax-chat`, {
    method: 'POST',
    signal,
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`MiniMax proxy error ${response.status}${detail ? `: ${detail}` : ''}`)
  }

  return response.json()
}

export function getMiniMaxContent(result: unknown): string {
  if (
