    ...
    >
      {internships.length === 0 ? (
        <div className="text-center py-10 animate-fade-in">
          ...
          <p className="text-gray-400 mb-3">暂无实习经历</p>
          <button
            onClick={addInternship}
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-1">
                  <button
                    onClick={() => moveInternship(item.id, 'up')}
                    disabled={index === 0}
                  ...
                  <button
                    onClick={() => moveInternship(item.id, 'down')}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-500 mb-1.5">公司</label>
                  <input
                    type="text"
                    value={item.company}
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1.5">职位</label>
                  <input
                    type="text"
                    value={item.position}
                    onChange={(e) => updateInternship(item.id, { position: e.target.value })}
                    placeholder="前端开发实习生"
                    className={inputClass}
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1.5">部门</label>
                  <input
                    type="text"
                    value={item.department}
                    onChange={(e) => updateInternship(item.id, { department: e.target.value })}
                    placeholder="增长部"
                    className={inputClass}
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => updateInternship(item.id, { startDate: e.target.value })}
                      placeholder="2023.06"
                      className={inputClass}
                    />
                    ...
                    <input
                      type="text"
                      value={item.endDate}
                      onChange={(e) => updateInternship(item.id, { endDate: e.target.value })}
                      placeholder="2023.09"
                      className={inputClass}
                    />
                  </div>
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1.5">地点</label>
                  <input
                    type="text"
                    value={item.location}
                    onChange={(e) => updateInternship(item.id, { location: e.target.value })}
                    placeholder="北京"
                    className={inputClass}
                  />
                </div>
                ...
                <div className="col-span-2">
                  ...
                  <RichTextEditor
                    value={item.content || ''}
                    onChange={(content) => updateInternship(item.id, { content })}
                    fontSize={item.contentFontSize || 10}
                    onFontSizeChange={(contentFontSize) =>
                      updateInternship(item.id, { contentFontSize })
                    }
                    placeholder="在这里统一输入该段实习经历正文，支持加粗、斜体、缩进、有序/无序列表。"
                  />
                </div>
              </div>
            </div>
