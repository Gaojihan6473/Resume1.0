...
【分析要求】
1. 匹配度评分（0-100）：以差异化信号匹配为主，综合技能关键词、经历相关性、业务/行业场景契合度、表达质量判断。普通产品经理能力只能作为基础分，不能因为覆盖常规职责就给高分。
2. scoreBreakdown 必须解释总分来源：skills=差异化技能匹配、experience=相关经历承接、keywords=特殊关键词覆盖、expression=差异化表达质量；每项 score 为 0-100，并给出一句具体 reason。
...
5. 不要只分析项目或经历开头；需要覆盖正文里的职责、技术栈、成果、指标、关键词表达，但只挑对差异化投递有实质影响的片段。
...
12. 建议必须聚焦成熟简历的差异化投递优化：特殊岗位关键词、相关经历表达、业务场景匹配、技能栈呈现、成果量化、职责与 JD 的对应关系、表达优先级。
13. problemReason 必须按这个逻辑写：JD 差异点是什么；简历现状是什么；为什么影响匹配。不要写泛泛的"不够突出""需要强化"。
14. suggestion 必须给具体修改策略：优先级、改法、避免事项。对已有事实，可给短语级或句子级改写方向；对无证据的 JD 要求，只能写"确认是否真实具备，有则补充具体证据；没有则不建议强行添加"，不要生成包含新事实的改写句。
15. 某个模块没有明显优化点时，summary 简短说明原因，status 返回"暂无问题"，suggestions 返回空数组。
16. 总建议数控制在 3-6 条。宁可少而准，不要为了凑数量覆盖普通职责。
...
3. 每个模块最多返回 2 条建议，总建议数控制在 3-6 条；如果没有高价值差异化建议，返回空数组。
...
6. matchScore 要客观真实，不要因为常规产品经理能力覆盖较多就过高评分。
